    ...
    <div className={`${classes["slider-bar"]} ${props.className}`}>
      <div className={classes.arrow}>
        <div
          ...
          onClick={clickHandleArrowLeft}
        >
          <NavigateBefore />
        </div>
        <div
          className={`${classes["arrow-icon"]} ${classes["arrow-icon-right"]}`}
          onClick={clickHandleArrowRight}
        >
          <NavigateNext />
        </div>
      </div>
      {/* list 10 product */}
      <ul
        className={classes["list-slider"]}
        style={{ transform: `translateX(${numberProduct * 25}%)` }}
        ref={listSlider}
      >
        {allVideo.map((child, index) => (
          <li key={index}>
            <Link
              to={`/detail/${child._id}`}
              style={{
                backgroundImage: `url(https://i.ytimg.com/vi/${child.idVideo}/hqdefault.jpg)`,
              }}
            ></Link>
          </li>
        ))}
