...
const TopProduct = () => {
  ...
  const renderChooseTop = chooseTopArr.map((child, index) => (
    <li
      onClick={() => handleClickChooseTop(index)}
      className={chooseTopItem === index ? classes.active : ""}
      key={index}
    >
      {child}
    </li>
  ...
  return (
    <div className={classes.topProduct}>
      <Typography className={classes.title}>Top Video hay</Typography>
      <ul className={classes.chooseTop}>{renderChooseTop}</ul>
      <ul className={classes.listProduct}>
        <li>
          <div>
            <Link
              ...
              style={{
                backgroundImage:
                  "url(https://i.ytimg.com/vi/CShWpR1A13Y/hq720.jpg)",
              }}
            ></Link>
          </div>
        ...
        <li>
          ...
          <div className={classes.nameProduct}>
            <p>
              SOFAR - BINZ DA POET | OFFICIAL MUSIC VIDEO SOFAR - BINZ DA POET |
